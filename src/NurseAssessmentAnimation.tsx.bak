  ...
  return (
    <div className="w-full min-h-[520px] bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl p-6">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <button onClick={() => setPlaying(p => !p)} className="inline-flex items-center gap-2 rounded-2xl px-4 py-2 border shadow-sm bg-white">
          {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
          {isPlaying ? "Pause" : "Play"}
        </button>
        <button onClick={() => { setPlaying(false); setStep(0); }} className="inline-flex items-center gap-2 rounded-2xl px-4 py-2 border shadow-sm bg-white">
          <RotateCcw className="w-4 h-4" /> Reset
        </button>
        <div className="flex items-center gap-2 bg-white rounded-2xl border px-3 py-2">
